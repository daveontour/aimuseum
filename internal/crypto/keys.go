...
// FindVisitorKeyringIDForPassword returns the sensitive_keyring.id of the first non-master seat
// that decrypts with password (ordered by id). Returns ok=false when none match.
func FindVisitorKeyringIDForPassword(ctx context.Context, pool *pgxpool.Pool, password, pepper string) (keyringID int64, ok bool, err error) {
	if password == "" {
		return 0, false, nil
	}
	...
	userKey := DeriveUserKey(password, pepper)
	q, args := uidFilter(ctx, `SELECT id, encrypted_dek FROM sensitive_keyring WHERE is_master = FALSE`, nil)
	q += ` ORDER BY id`
	rows, err := pool.Query(ctx, q, args...)
	if err != nil {
		return 0, false, fmt.Errorf("query sensitive_keyring: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var id int64
		var encDek []byte
		if err := rows.Scan(&id, &encDek); err != nil {
			return 0, false, err
		}
		var dek string
		if err := pool.QueryRow(ctx, `SELECT pgp_sym_decrypt($1, $2)`, encDek, userKey).Scan(&dek); err == nil && dek != "" {
			return id, true, nil
		}
	}
