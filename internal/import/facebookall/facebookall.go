...
func ClearFacebookAllDataForUser(ctx context.Context, pool *pgxpool.Pool, userID int64) error {
	if userID == 0 {
	...
	_, err = tx.Exec(ctx, `DELETE FROM messages WHERE service = 'Facebook Messenger' AND user_id = $1`, userID)
	if err != nil {
	...
	_, err = tx.Exec(ctx, `DELETE FROM facebook_albums WHERE user_id = $1`, userID)
	if err != nil {
		return fmt.Errorf("failed to clear Facebook albums: %w", err)
	}

	_, err = tx.Exec(ctx, `DELETE FROM locations WHERE source = 'facebook' AND user_id = $1`, userID)
	if err != nil {
	...
	_, err = tx.Exec(ctx, `DELETE FROM facebook_posts WHERE user_id = $1`, userID)
	if err != nil {
