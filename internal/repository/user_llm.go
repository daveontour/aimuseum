...
func mergeLLMStoredWithPatch(cur UserLLMStored, p UserLLMPatch) UserLLMStored {
	...
	return UserLLMStored{GeminiAPIKey: gk, AnthropicAPIKey: ak, GeminiModel: gm, ClaudeModel: cm, TavilyAPIKey: tk}
}
...
func (r *UserRepo) GetUserLLMStored(ctx context.Context, userID int64) (*UserLLMStored, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		userID,
	).Scan(&s.GeminiAPIKey, &s.AnthropicAPIKey, &s.GeminiModel, &s.ClaudeModel, &s.TavilyAPIKey)
	if err != nil {
		return nil, err
	}
	return &s, nil
